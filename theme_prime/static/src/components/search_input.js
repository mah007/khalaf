...
import { Component, onWillStart, onWillUpdateProps, toRaw, useState, useRef } from "@odoo/owl";
...
export class TpSearchInput extends CoreComponent {
    ...
    get resModelInfo() {
        ...
        'product.attribute.value': { title: this.props.brand ? _t("Brand")  : _t("Attribute")},
        'dr.product.label': { title: _t("Label")},
...
export class TpDomainComponent extends AbstractComponent {
    ...
    get fieldsList () {
        return [{'type': 'text', 'name': 'name', 'label': _t('Name') }, {'type': 'many2many', 'name': 'public_categ_ids', 'label': _t('Category'), 'relationModel': 'product.public.category' }, {'type': 'many2one', 'name': 'dr_brand_value_id', 'label': _t('Brand'), 'relationModel': 'product.attribute.value', 'extras': { 'brands': true } }, {'type': 'many2one', 'name': 'attribute_line_ids.value_ids', 'label': _t('Attributes'), 'relationModel': 'product.attribute.value' }, {'type': 'many2one', 'name': 'dr_label_id', 'label': _t('Label'), 'relationModel': 'dr.product.label' }, {'type': 'integer', 'name': 'list_price', 'label': _t('Price') }, {'type': 'many2one', 'name': 'product_tag_ids', 'label': _t('Tags'), 'relationModel': 'product.tag', 'is_multi_website': true }, {'type': 'boolean', 'name': 'dr_has_discount', 'label': _t('Discount') }];
    }
    get domainTemplates() {
