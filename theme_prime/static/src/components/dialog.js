...
export class TpSnippetConfigDialog extends Component {
    setup() {
        ...
        const { metadata: { title }, name } = this.tpWebsite.currentWebsite;
        this.title = title;
    ...
    prepareComponents(snippetData, hardRestore) {
        ...
        Object.keys(data.components).forEach(comp => {
            ...
            this.supportedComponents.components.push({ ...Components[comp], props: { extras: { ...data.components[comp], templateID: this.previewState.activeStep }, componentData: value, allData: this.state, HasRecords: this.previewState.HasRecords }, name: comp});
        });
    }
    _onPreviewLoaded(){
